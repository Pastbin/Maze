class Maze {
  ...
  static findTheWayOut(arrayFromMaze) {
    ...
    while (!mazeComplete) {
      this.getEveryPixel(arrayFromMaze, (i, ii, direction) => {
        if (((arrayFromMaze[i][ii] >= 50 && !count) || (arrayFromMaze[i][ii] >= 3 && count)) &&
          (!count || (count - arrayFromMaze[i][ii] <= 3 && count - arrayFromMaze[i][ii] >= 0))) {
  ...
  static isMazeInRow(arr, color) {
    return arr[color] + arr[color + 1] + arr[color + 2] < 700 ? 1 : 0
  }
...
const timeWatcher = document.querySelector('#time')
...
function fileHandler(e) {
  ...
  image.onload = () => {
    ...
    timeWatcher.textContent = `Время обработки: ${Date.now() - timer}мсек`
  }
  if (e.target.files[0]) {
